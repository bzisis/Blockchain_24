...
///
/// This struct combines a `Config` object with various preset structs corresponding to different
/// fork versions. It also includes extra fields for future hard forks, allowing graceful
/// decoding of additional fields.
///
...
pub struct ConfigAndPreset {
    ...
    /// Altair preset fields.
    #[serde(flatten)]
    pub altair_preset: AltairPreset,
    /// Bellatrix preset fields.
    #[serde(flatten)]
    pub bellatrix_preset: BellatrixPreset,
    /// Capella preset fields.
    #[serde(flatten)]
    pub capella_preset: CapellaPreset,
    /// Deneb preset fields (only available for Deneb and Electra variants).
    #[superstruct(only(Deneb, Electra))]
    #[serde(flatten)]
    pub deneb_preset: DenebPreset,
    /// Electra preset fields (only available for Electra variant).
    #[superstruct(only(Electra))]
    #[serde(flatten)]
    pub electra_preset: ElectraPreset,
    /// Additional fields that can be dynamically added or decoded.
    ///
...
impl ConfigAndPreset {
    /// Constructs a `ConfigAndPreset` instance from a `ChainSpec` and optional `ForkName`.
    ///
    /// This function gathers runtime configuration and preset values from the `ChainSpec` based
    /// on the specified or inferred fork.
...
///
/// This function retrieves constants from a `ChainSpec` and organizes them into a hashmap,
/// ready to be included in `ConfigAndPreset` as `extra_fields`.
pub fn get_extra_fields(spec: &ChainSpec) -> HashMap<String, Value> {
    ...
    hashmap! {
        ...
        "domain_contribution_and_proof".to_uppercase() => u32_hex(spec.domain_contribution_and_proof),
        "domain_sync_committee".to_uppercase() => u32_hex(spec.domain_sync_committee),
        "domain_sync_committee_selection_proof".to_uppercase() => u32_hex(spec.domain_sync_committee_selection_proof),
        "sync_committee_subnet_count".to_uppercase() => altair::SYNC_COMMITTEE_SUBNET_COUNT.to_string().into(),
        "target_aggregators_per_sync_subcommittee".to_uppercase() => altair::TARGET_AGGREGATORS_PER_SYNC_SUBCOMMITTEE.to_string().into(),
        // Deneb
