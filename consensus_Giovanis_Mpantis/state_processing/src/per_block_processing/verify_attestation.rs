...
/// # Arguments
///
/// * `state` - The beacon state to validate against.
/// * `attestation` - The attestation to validate.
/// * `ctxt` - Mutable context for consensus-specific information.
/// * `verify_signatures` - Flag indicating whether to verify aggregate signatures.
/// * `spec` - Chain specification parameters.
///
/// # Returns
///
/// * `Result<&'ctxt IndexedAttestation<E>>` - A reference to the validated indexed attestation on success.
...
///
/// * `attestation` - The attestation to validate.
/// * `state` - The beacon state containing relevant context.
///
/// # Returns
///
/// * `Result<()>` - `Ok(())` if the attestation's target epoch and source checkpoint are valid, otherwise a descriptive `Err`.
