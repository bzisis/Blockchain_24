...
///
/// This function updates the validator registry based on activation eligibility and ejections.
///
...
pub fn process_registry_updates<E: EthSpec>(
    ...
    // Update each validator in `indices_to_update`.
...
/// Processes registry updates in a slower, single-pass manner.
///
/// This function processes registry updates in a single pass using `process_epoch_single_pass`.
/// It enables only the `registry_updates` phase and disables all other processing phases.
