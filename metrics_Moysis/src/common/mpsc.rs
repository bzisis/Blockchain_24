...
// Importing the necessary traits and types for asynchronous streams.
use futures::Stream;

// Importing the Counter type from the metrics crate for tracking metrics.
use metrics::Counter;

// Importing the Metrics derive macro for automatically generating metric-related code.
use reth_metrics_derive::Metrics;

// Importing standard library types for pinning and task management.
use std::{
    pin::Pin,                    // Pinning ensures that the data is not moved in memory.
    task::{ready, Context, Poll}, // Context and Poll are used for asynchronous task polling.
};

// Importing types and modules from the tokio crate for asynchronous multi-producer, single-consumer channels.
use tokio::sync::mpsc::{
    self,                          // mpsc module for multi-producer, single-consumer channels.
    error::{                       // Importing error types related to mpsc operations.
        SendError,                 // Error type for send operations.
        TryRecvError,              // Error type for try receive operations.
        TrySendError               // Error type for try send operations.
    },
    OwnedPermit                    // Permit type for reserved sends in bounded channels.
};

// Importing additional synchronization utilities from tokio_util.
use tokio_util::sync::{
    PollSendError,                 // Error type for poll send operations.
    PollSender                     // Sender type that supports poll-based sending.
};
...
impl<T> Clone for UnboundedMeteredSender<T> {
    /// Creates a new `UnboundedMeteredSender` instance by cloning the existing one.
    ///
    /// This method duplicates the underlying `UnboundedSender` and the associated metrics,
    /// ensuring that the new instance has the same state as the original.
    fn clone(&self) -> Self {
...
impl<T> Stream for UnboundedMeteredReceiver<T> {
    ...
    /// Polls to receive the next item from the stream.
    ///
    /// This method is called by the runtime to attempt to resolve the next item in the stream.
    /// It delegates to the `poll_recv` method, which polls to receive the next message on the channel.
    ///
    /// # Arguments
    ///
    /// * `self` - A pinned reference to the `UnboundedMeteredReceiver`.
    /// * `cx` - The context to poll with.
...
impl<T> Clone for MeteredSender<T> {
    /// Creates a new `MeteredSender` instance by cloning the existing one.
    ///
    /// This method duplicates the underlying `Sender` and the associated metrics,
    /// ensuring that the new instance has the same state as the original.
    fn clone(&self) -> Self {
        Self { sender: self.sender.clone(), metrics: self.metrics.clone() }
    }
...
impl<T> Stream for MeteredReceiver<T> {
    ...
    /// # Arguments
    ///
    /// * `self` - A pinned reference to the `MeteredReceiver`.
    /// * `cx` - The context to poll with.
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        self.poll_recv(cx)
    }
...
impl<T> Clone for MeteredPollSender<T> {
    /// Creates a new `MeteredPollSender` instance by cloning the existing one.
    ///
    /// This method duplicates the underlying `PollSender` and the associated metrics, ensuring that
    /// the new instance has the same state as the original.
    fn clone(&self) -> Self {
        Self { sender: self.sender.clone(), metrics: self.metrics.clone() }
    }
}
