...
/// Processes all operations in a Beacon block body, updating the Beacon state accordingly.
///
/// # Errors
///
...
/// Module containing base functions for attestations processing.
pub mod base {
    use super::*;
...
/// Module containing functions specific to the Altair and Deneb fork processing of attestations.
pub mod altair_deneb {
    use super::*;
    use crate::common::update_progressive_balances_cache::update_progressive_balances_on_attestation;

    /// Processes attestations for the Altair and Deneb forks.
    ///
    /// Returns `Ok(())` if all attestations are processed successfully.
    pub fn process_attestations<E: EthSpec>(
        state: &mut BeaconState<E>,
        attestations: &[Attestation<E>],
        verify_signatures: VerifySignatures,
    ...
    /// Processes a single attestation for the Altair and Deneb forks.
    ///
    /// Returns `Ok(())` if the attestation is processed successfully.
...
///
/// Returns `Ok(())` if the deposit is successfully processed.
