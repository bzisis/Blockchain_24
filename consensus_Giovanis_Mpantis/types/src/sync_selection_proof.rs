...
impl SyncSelectionProof {
    ...
    /// Checks if this `SyncSelectionProof` is an aggregator.
    pub fn is_aggregator<E: EthSpec>(&self) -> Result<bool, ArithError> {
        self.is_aggregator_from_modulo(Self::modulo::<E>()?)
    }

    /// Checks if this `SyncSelectionProof` is an aggregator given a modulo.
    ...
    /// Verifies the `SyncSelectionProof` against the given parameters.
