...
impl TotalBalances {
    /// Creates a new `TotalBalances` instance with initial values based on the provided `spec`.
...
impl ValidatorStatuses {
    /// Initializes a new instance of `ValidatorStatuses` using the provided `state` and `spec`.
    ///
    /// This function determines active validators and computes total balances for the current and
    /// previous epochs.
    ///
    /// Spec v0.12.1
    ...
    /// Processes attestations from the given `state`, updating `statuses` and `total_balances`.
    ///
    /// Spec v0.12.1
    pub fn process_attestations<E: EthSpec>(
            ...
            .chain(base_state.current_epoch_attestations.iter())
        {
            ...
            // Profile this attestation, updating the total balances and generating a
