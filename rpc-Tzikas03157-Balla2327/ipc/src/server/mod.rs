...
pub struct IpcServer<HttpMiddleware = Identity, RpcMiddleware = Identity> {
    ...
    endpoint: String,
    /// Provides unique identifiers for requests.
    id_provider: Arc<dyn IdProvider>,
    /// Configuration settings for the server.
    cfg: Settings,
    /// Middleware for RPC services.
        ...
        + 'static,
{
    ...
    /// Inner function to start the server.
...
/// Represents the outcome of attempting to accept a new connection.
...
/// # Arguments
///
/// * `listener` - The local socket listener.
/// * `stopped` - A future that resolves when the server is stopped.
///
/// # Returns
///
/// An `AcceptConnection` indicating the outcome.
...
/// Error thrown when the server couldn't be started.
...
impl RpcServiceBuilder<Identity> {
    /// Creates a new [`RpcServiceBuilder`].
    pub fn new() -> Self {
        Self(tower::ServiceBuilder::new())
    }
}
...
impl<L> RpcServiceBuilder<L> {
    /// Optionally adds a new layer `T` to the [`RpcServiceBuilder`].
    ///
    ...
    /// Adds a new layer `T` to the [`RpcServiceBuilder`].
    ///
    ...
    /// Adds a [`tower::Layer`] built from a function that accepts a service and returns another service.
    ///
    ...
    /// Adds a logging layer to [`RpcServiceBuilder`].
    ///
    ...
    /// Wraps the service `S` with the middleware.
    ...
    for<'a> <RpcMiddleware as Layer<RpcService>>::Service: Send + Sync + 'static + RpcServiceT<'a>,
{
    ...
    /// Opens the door for backpressure implementation.
    ...
    /// # Arguments
    ///
    /// * `request` - The incoming RPC request as a string.
    ///
    /// # Returns
    ///
    ...
    fn call(&mut self, request: String) -> Self::Future {
        ...
        // Even if the underlying RPC handler spawns the actual work or does a lot of async work,
        // any additional overhead performed by `handle_request` can result in I/O latencies.
        // For example, tracing calls are relatively CPU expensive on serde::serialize alone,
        // moving this work to a separate task takes the pressure off the connection so all
        // concurrent responses are also serialized concurrently and the connection can focus on
        // read+write.
...
/// Represents the parameters required to process a connection.
...
///
/// This function creates the IPC connection, sets up the service, and spawns a new task to handle
/// the connection.
///
/// # Arguments
///
/// * `params` - The parameters required to process the connection.
...
/// Drives the IPC connection forward.
///
/// This function handles the concurrent processing of the IPC connection, managing
/// read and write operations, and gracefully shutting down the connection.
///
/// # Arguments
///
/// * `ipc` - The IPC connection.
/// * `service` - The service to handle the requests.
/// * `stop_handle` - The handle to stop the server.
...
impl<HttpMiddleware, RpcMiddleware> Builder<HttpMiddleware, RpcMiddleware> {
    ...
    /// Sets the maximum size of a log.
    pub const fn max_log_length(mut self, size: u32) -> Self {
        self.settings.max_log_length = size;
        self
    }

    /// Sets the maximum number of connections allowed. Default is 100.
    ...
    ///
    /// The server enforces backpressure which means that `n` messages can be buffered,
    /// and if the client can't keep up with the server, the server will back off and only accept new messages
    /// once the client reads pending messages.
    ///
    /// # Panics
    ///
    ...
    /// Configures a custom [`tokio::runtime::Handle`] to run the server on.
    ///
    /// Default: [`tokio::spawn`]
    ...
    /// Configures a custom `subscription ID` provider for the server to use
    /// when getting new subscription calls.
    ///
    ...
    /// Configures a custom [`tower::ServiceBuilder`] middleware for composing layers to be applied
    /// to the RPC service.
    ///
    ...
    /// Enables middleware that is invoked on every JSON-RPC call.
    ///
    /// The middleware itself is very similar to the `tower middleware` but
    /// it has a different service trait which takes `&self` instead of `&mut self`,
    /// which means that you can't use built-in middleware from tower.
    ///
    /// Another consequence of `&self` is that you must wrap any middleware state in
    ...
    /// Finalizes the configuration of the server. Consumes the [`Builder`].
...
mod tests {
    ...
    /// Pipes items from a stream with a bounded buffer to a pending subscription sink.
    ///
    /// # Arguments
    ///
    /// * `pending` - The pending subscription sink.
    /// * `stream` - The stream of items.
        ...
        loop {
            match select(closed, stream.next()).await {
                ...
                Either::Right((Some(Ok(item)), c)) => {
                    ...
                    // NOTE: This will block until there is a spot in the queue.
                    // You might want to do something smarter if it's critical that "the most recent item"
                    // must be sent when it is produced.
    ...
    /// Test setting the maximum response body size.
    #[tokio::test]
    ...
    /// Test setting the maximum request body size.
    #[tokio::test]
    ...
    /// Test setting the maximum number of connections.
    #[tokio::test]
    ...
    /// Test the available RPC modules.
    #[tokio::test]
    ...
    async fn test_rpc_middleware() {
            ...
            S: Send + Sync + RpcServiceT<'a>,
        {
            ...
            fn call(&self, mut req: Request<'a>) -> Self::Future {
                // Redirect all calls that aren't `say_hello` to `say_goodbye`.
