...
///
/// This function upgrades a given `Deneb` state to an `Electra` state by transforming its fields
/// and preparing it for the `Electra` protocol.
///
/// # Arguments
///
/// * `pre_state` - A mutable reference to the `Deneb` state to be upgraded.
/// * `spec` - The chain specification used for upgrading.
///
/// # Errors
///
/// Returns an error if any operation during the upgrade process fails.
///
/// # Examples
///
/// ```
/// use types::{BeaconState, ChainSpec};
/// use my_crate::upgrade_to_electra;
///
/// let mut state: BeaconState<MyEthSpec> = BeaconState::default(); // Initialize your state
/// let spec: ChainSpec = ChainSpec::default(); // Initialize your chain specification
///
/// match upgrade_to_electra(&mut state, &spec) {
///     Ok(()) => println!("State successfully upgraded to Electra!"),
