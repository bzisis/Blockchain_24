...
impl MerkleTree {
    ...
    /// Get all finalized hashes in this Merkle tree.
    ...
    /// Generate a Merkle proof for the leaf at `index` in this Merkle tree.
    ///
    /// Returns the root hash of the tree and a vector of hashes of the nodes
    /// on the path from the leaf to the root (in bottom-up order).
    ...
    /// Print a textual representation of this Merkle tree node.
    pub fn print_node(&self, mut space: u32) {
        ...
        let (pair, text) = match self {
            ...
            MerkleTree::Zero(depth) => (
                None,
                format!("Zero[{}]({})", depth, H256::from_slice(&ZERO_HASHES[*depth])),
            ),
            MerkleTree::Finalized(hash) => (None, format!("Finalized({})", hash)),
        };
