...
impl EngineTypes for OptimismEngineTypes {
    ...
    /// Validates version-specific fields in the payload or attributes.
    ///
    /// This method ensures that withdrawals are correctly validated based on the
    /// chain specification and the version of the engine API message.
    ///
    /// ### Parameters
    /// - `chain_spec`: The [`ChainSpec`] for the node.
    /// - `version`: The engine API message version.
    /// - `payload_or_attrs`: Payload or attributes to validate.
    ///
    /// ### Returns
    /// Returns `Ok(())` if validation is successful, otherwise returns an [`EngineObjectValidationError`].
