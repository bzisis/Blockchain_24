...
/// Maximum number of fork versions against which a message can be verified.
    ...
    E: EthSpec,
{
    ...
    /// Extracts the inner operation from `SigVerifiedOp`.
    ...
    /// Checks if the signature of the operation is still valid against the current fork.
    ...
    /// Returns the first fork version this message was verified against, if available.
    ///
...
pub trait VerifyOperation<E: EthSpec>: Encode + Decode + Sized {
    ...
    /// Validates and wraps the operation into a `SigVerifiedOp`.
    fn validate(
        self,
        state: &BeaconState<E>,
        spec: &ChainSpec,
    ...
    /// Returns the epochs at which parts of this message were verified.
    ///
...
pub trait VerifyOperationAt<E: EthSpec>: VerifyOperation<E> + Sized {
    /// Validates and wraps the operation into a `SigVerifiedOp`, verifying at a specific epoch.
    fn validate_at(
        self,
        state: &BeaconState<E>,
