...
//!
//! This module defines the `NodeConfig` struct and its associated methods to manage and customize
//! various parameters essential for launching an Ethereum node. Key functionalities include:
//!
//! - **Node Configuration**: Structured configuration options for data directories, chain specifications,
//!   networking, RPC servers, transaction pools, payload builders, debugging, databases, and more.
//!
//! - **External Integrations**: Integration with external crates and APIs like Prometheus for metrics
//!   handling (`metrics_exporter_prometheus`), tracing for logging (`tracing`), and other Ethereum-related
//!   crates (`reth_*` crates).
//!
//! - **Prometheus Metrics**: Setting up and serving Prometheus metrics endpoints (`start_metrics_endpoint`),
//!   enabling the node to expose various operational metrics.
//!
//! - **Network Interaction**: Management of Ethereum network interactions, including fetching headers,
//!   handling node stages (`StageId`), and ensuring robust header retrieval from the network.
//!
//! - **Development and Testing**: Support for development mode (`dev` method), configuration of test
//!   environments (`test` method), and handling of test-specific configurations (e.g., using random ports).
//!
//! - **Error Handling and Asynchronous Operations**: Error handling with the `eyre` crate, asynchronous
//!   operations using `async fn` and `await`, and management of asynchronous tasks (`TaskExecutor`).
//!
//! - **Utility Functions and Constants**: Utility functions (`kzg_settings`, `install_prometheus_recorder`)
//!   and constants (`PROMETHEUS_RECORDER_HANDLE`) used throughout the node configuration setup.
