...
pub enum EpochProcessingSummary<E: EthSpec> {
    ...
    /// Summary for the Altair epoch.
    Altair {
        /// Cache for progressive balances.
        progressive_balances: ProgressiveBalancesCache,
        /// Total active balance for the current epoch.
        current_epoch_total_active_balance: u64,
        /// Summary of participation for the epoch.
        participation: ParticipationEpochSummary<E>,
        /// Sync committee for the epoch.
        sync_committee: Arc<SyncCommittee<E>>,
    },
}

/// Provides a summary of validator participation for a specific epoch.
#[derive(PartialEq, Debug)]
...
impl<E: EthSpec> ParticipationEpochSummary<E> {
    /// Creates a new `ParticipationEpochSummary`.
    pub fn new(
        validators: List<Validator, E::ValidatorRegistryLimit>,
        previous_epoch_participation: List<ParticipationFlags, E::ValidatorRegistryLimit>,
        current_epoch_participation: List<ParticipationFlags, E::ValidatorRegistryLimit>,
    ...
    /// Checks if a validator is active and not slashed in the given epoch.
    ...
    /// Checks if a validator was unslashed and participating in the previous epoch.
    ...
    /// Checks if a validator is unslashed and participating in the current epoch.
...
impl<E: EthSpec> EpochProcessingSummary<E> {
    /// Updates some Prometheus metrics with values from `self`.
