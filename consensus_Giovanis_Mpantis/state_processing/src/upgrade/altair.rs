...
/// * `pending_attestations` - List of pending attestations for the epoch.
/// * `spec` - Chain specification defining consensus rules.
///
/// # Errors
///
/// Returns an error if there are issues building the committee cache or processing attestations.
...
///
/// This function upgrades the given base state to an Altair state by modifying its fields
/// according to the Altair upgrade specifications.
///
/// # Arguments
///
/// * `pre_state` - The mutable reference to the base beacon state to be upgraded.
/// * `spec` - Chain specification defining consensus rules.
///
/// # Errors
///
/// Returns an error if there are issues in upgrading the state or initializing caches.
