...
//!
//! The `BenchmarkArgs` struct includes the following parameters:
//! - `from`: Specifies the starting block number for the benchmark.
//! - `to`: Specifies the ending block number for the benchmark.
//! - `auth_jwtsecret`: Path to a JWT secret for the authenticated engine-API RPC server.
//! - `engine_rpc_url`: The URL of the engine's RPC server, with a default value of `http://localhost:8551`.
//! - `output`: The path to the output directory for saving granular benchmark results.
//!
//! This module also includes unit tests to verify that the default arguments are parsed correctly.
