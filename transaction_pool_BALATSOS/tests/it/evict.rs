...
// Contents: 
// 1) "only_blobs_eviction" -> Ensures blob transactions can be inserted into the pool and truncated correctly
// 2) "mixed_eviction" -> Tests the insertion of a mix of different transaction types
// 3) "nonce_gaps_eviction" -> Checks the behavior when inserting transactions with nonce gaps
// 4) "PoolConfig" -> Configures the transaction pool's limits
// 5) "BlockInfo" -> Contains information about the current block
// 6) "MockTransactionDistribution" -> Distributes mock transactions with various configurations
// 7) "MockTransactionRatio" -> Defines the percentage of each type of transaction
// 8) "MockFeeRange" -> Defines the fee ranges for mock transactions

//! Transaction pool eviction tests.

//
/// This module contains tests to ensure the correct behavior of the transaction pool
/// when dealing with various types of transactions, particularly focusing on eviction
/// policies and handling nonce gaps.
///
...
///
/// Steps:
/// 1. Configures a transaction pool with small limits.
/// 2. Inserts a large number of blob transactions.
/// 3. Ensures the pool maintains its size constraints after each insertion.
///
...
async fn only_blobs_eviction() {
    ...
    // Initialize the test transaction pool with the configured limits
...
/// This test checks that various types of transactions can be inserted into the pool.
/// It ensures that the transactions pass validation and the pool maintains its limits.
///
/// Steps:
/// 1. Configures a transaction pool with specific limits.
/// 2. Inserts a mix of different transaction types.
/// 3. Ensures the pool maintains its size constraints after each insertion.
///
#[tokio::test(flavor = "multi_thread")]
async fn mixed_eviction() {
    ...
    for sender_amt in &senders {
        ...
        // Set the fee range for the mock transactions
        let fee_range = MockFeeRange {
...
/// This test checks that various types of transactions, including those with nonce gaps,
/// can be inserted into the pool. It ensures that the transactions pass validation and
/// the pool maintains its limits.
///
/// Steps:
/// 1. Configures a transaction pool with specific limits.
/// 2. Inserts a mix of different transaction types, some with nonce gaps.
/// 3. Ensures the pool maintains its size constraints after each insertion.
///
#[tokio::test(flavor = "multi_thread")]
