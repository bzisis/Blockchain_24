...
pub fn extract_l1_info(block: &Block) -> Result<L1BlockInfo, OptimismBlockExecutionError> {
    // Attempt to retrieve the transaction data of the first transaction in the block
    ...
    // Check if the transaction data length is at least 4 bytes
    ...
    // Determine if the transaction corresponds to Ecotone or Bedrock hardfork based on the selector
    if l1_info_tx_data[0..4] == L1_BLOCK_ECOTONE_SELECTOR {
        // Parse the transaction data assuming it's for the Ecotone hardfork
        parse_l1_info_tx_ecotone(l1_info_tx_data[4..].as_ref())
    } else {
        // Parse the transaction data assuming it's for the Bedrock hardfork
...
pub fn parse_l1_info_tx_bedrock(data: &[u8]) -> Result<L1BlockInfo, OptimismBlockExecutionError> {
    // Validate that the transaction data length is exactly 256 bytes
    if data.len() != 256 {
        return Err(OptimismBlockExecutionError::L1BlockInfoError {
    ...
    // Construct and return the L1BlockInfo struct
...
impl RethL1BlockInfo for L1BlockInfo {
    /// Calculates the data gas cost for an L1 transaction.
    ///
    /// ### Parameters
    /// - `chain_spec`: The [`ChainSpec`] for the node.
    /// - `timestamp`: The timestamp of the current block.
    ...
    ///
    /// ### Returns
    /// Returns the calculated data gas cost.
    fn l1_data_gas(
        &self,
        chain_spec: &ChainSpec,
        timestamp: u64,
        ...
        // Determine the specific Optimism hardfork based on the timestamp
    ...
    /// Calculates the transaction data fee for an L1 transaction.
    ///
    /// ### Parameters
    /// - `chain_spec`: The [`ChainSpec`] for the node.
    /// - `timestamp`: The timestamp of the current block.
    ...
    ///
    /// ### Returns
    /// Returns the calculated transaction data fee.
    fn l1_tx_data_fee(
        &self,
        chain_spec: &ChainSpec,
        timestamp: u64,
...
impl RethL1BlockInfo for L1BlockInfo {
    /// Computes the transaction data fee for an L1 transaction.
    ///
    /// This method calculates the fee based on the chain specification, timestamp,
    /// input data, and whether the transaction is a deposit.
    ///
    /// ### Takes
    /// - `chain_spec`: The [`ChainSpec`] for the node.
    /// - `timestamp`: The timestamp of the current block.
    /// - `input`: The calldata of the transaction.
    /// - `is_deposit`: Indicates if the transaction is a deposit transaction.
    ///
    /// ### Returns
    /// Returns the calculated transaction data fee as [`U256`].
    ///
    /// ### Errors
    /// This method returns an error if the chain specification does not have any active
    /// Optimism hardforks at the provided timestamp.
    fn l1_tx_data_fee(
        &self,
        chain_spec: &ChainSpec,
        timestamp: u64,
        ...
        // If it's a deposit transaction, return zero fee.
        if is_deposit {
            return Ok(U256::ZERO);
        }

        // Determine which Optimism hardfork is active at the given timestamp.
        let spec_id = if chain_spec.is_fork_active_at_timestamp(OptimismHardfork::Fjord, timestamp) {
            SpecId::FJORD
        } else if chain_spec.is_fork_active_at_timestamp(OptimismHardfork::Ecotone, timestamp) {
            SpecId::ECOTONE
            ...
            // If no Optimism hardforks are active, return an error indicating the issue.
            return Err(OptimismBlockExecutionError::L1BlockInfoError {
                message: "Optimism hardforks are not active".to_string(),
            }
            .into());
        ...
        // Calculate and return the transaction data gas cost based on the identified hardfork.
        Ok(self.calculate_tx_l1_cost(input, spec_id))
    }

    /// Computes the data gas cost for an L2 transaction.
    ///
    /// This method calculates the gas cost required to store the provided transaction data
    /// based on the chain specification and timestamp. It handles different Optimism hardforks
    /// to determine the appropriate gas cost calculation.
    ///
    /// ### Takes
    /// - `chain_spec`: The [`ChainSpec`] for the node.
    /// - `timestamp`: The timestamp of the current block.
    ...
    ///
    /// ### Returns
    /// Returns the computed data gas cost as [`U256`].
    ///
    /// ### Errors
    /// This method returns an error if the chain specification does not have any active
    /// Optimism hardforks at the provided timestamp.
    fn l1_data_gas(
        ...
        // Calculate and return the data gas cost based on the identified hardfork.
        Ok(self.data_gas(input, spec_id))
    }
}
    ...
    DB: revm::Database,
{
    ...
    // previous block timestamp (heuristically, block time is not perfectly constant at 2s),
    // and the chain is an optimism chain, then we need to force-deploy the create2 deployer contract.
            ...
            .is_fork_active_at_timestamp(OptimismHardfork::Canyon, timestamp.saturating_sub(2))
    {
        ...
        // Load the create2 deployer account from the database cache.
        let acc = db.load_cache_account(CREATE_2_DEPLOYER_ADDR)?;

        // Update the account information with the create2 deployer codehash and bytecode.
...
mod tests {
    ...
    /// Tests the extraction of L1 block information from a sanity block.
    #[test]
    fn sanity_l1_block() {
        // Import necessary types and utilities from reth_primitives.
        use reth_primitives::{hex_literal::hex, Bytes, Header, TransactionSigned};

        // Mock transaction bytes representing an L1 block.
