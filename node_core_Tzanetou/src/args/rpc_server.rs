...
//! 
//! The `RpcServerArgs` struct encapsulates various configuration options for HTTP-RPC, WS-RPC, IPC-RPC, and authentication related RPC servers.
//! 
//! It includes options for:
//! - Enabling and configuring HTTP-RPC (`http`) and WS-RPC (`ws`) servers, including server address, port, allowed origins, and enabled API modules.
//! - Configuring IPC-RPC (`ipcdisable`, `ipcpath`) and authentication RPC (`auth_addr`, `auth_port`, `auth_jwtsecret`, `auth_ipc`, `auth_ipc_path`).
//! - Setting JWT secrets (`auth_jwtsecret`, `rpc_jwtsecret`) and maximum payload sizes (`rpc_max_request_size`, `rpc_max_response_size`).
//! - Tuning RPC performance parameters such as maximum subscriptions per connection (`rpc_max_subscriptions_per_connection`), maximum connections (`rpc_max_connections`), and maximum tracing requests (`rpc_max_tracing_requests`).
//! - Configuring gas limits (`rpc_gas_cap`) and other specialized RPC configurations (`rpc_max_blocks_per_filter`, `rpc_max_logs_per_response`).
//! 
//! The `RpcServerArgs` struct also provides methods for adjusting ports based on instance numbers (`adjust_instance_ports`), enabling specific server types (`with_http`, `with_ws`, `with_auth_ipc`), and setting ports to use random unused values (`with_http_unused_port`, `with_ws_unused_port`, `with_auth_unused_port`).
//! 
//! Tests are included to validate the parsing and default behavior of `RpcServerArgs`, ensuring correct configuration when parsed with various command-line arguments.
