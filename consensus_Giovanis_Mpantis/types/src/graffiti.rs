...
/// Length of the graffiti field in bytes.
...
impl Graffiti {
    /// Converts the graffiti bytes to a UTF-8 string, replacing invalid UTF-8 sequences.
...
/// Wrapper struct around graffiti string for serialization purposes.
#[derive(Debug, Clone, PartialEq, Serialize, Default)]
#[serde(transparent)]
pub struct GraffitiString(String);

impl GraffitiString {
    /// Creates an empty `GraffitiString`.
    pub fn empty() -> Self {
        Self(String::new())
    }
}
...
/// Serialization and deserialization functions for `Graffiti` using hexadecimal encoding.
pub mod serde_graffiti {
    use super::*;

    /// Serialize `bytes` to a hexadecimal string.
    ...
    /// Deserialize a hexadecimal string into `GRAFFITI_BYTES_LEN` bytes.
