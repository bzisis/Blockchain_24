    ...
    Eth: EthTransactions + LoadPendingBlock + Call + 'static,
{
    ...
    /// # Arguments
    ///
    /// * `bundle` - The bundle of transactions to simulate.
    ///
    /// # Returns
    ///
    /// An `EthResult` containing the response with the results of the transaction simulation.
    pub async fn call_bundle(&self, bundle: EthCallBundle) -> EthResult<EthCallBundleResponse> {
        ...
        // Check for empty transactions in the bundle
        if txs.is_empty() {
        ...
        // Check for missing block number in the bundle
        if block_number == 0 {
            return Err(EthApiError::InvalidParams(
        ...
        // Recover transactions and validate their components
        ...
        // Adjust the timestamp for the next block
        ...
        self.inner
            ...
            .spawn_with_state_at_block(at, move |state| {
                ...
                while let Some((tx, signer)) = transactions.next() {
                    ...
                    // Apply the state changes of this call before executing the next call
...
/// Implements the `EthCallBundleApiServer` trait for `EthBundle`.
/// This trait provides methods for handling Ethereum bundle call requests.
...
{
    /// Simulates a bundle of transactions at the top of a given block number with the state of
    /// another (or the same) block.
    ///
    /// # Arguments
    ///
    /// * `request` - The bundle of transactions to simulate.
    ///
    /// # Returns
    ///
    /// An `RpcResult` containing the response with the results of the transaction simulation.
...
/// Implements the `Debug` trait for `EthBundle`.
...
/// Implements the `Clone` trait for `EthBundle`.
