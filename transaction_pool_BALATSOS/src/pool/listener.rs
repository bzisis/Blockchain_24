...
// Contents:
// 1) broadcast_event(&mut self, hash: &TxHash, event: TransactionEvent, pool_event: FullTransactionEvent<T>) -> Broadcasts an event to all listeners 
//    for a specific transaction hash and all transactions
// 2) subscribe(&mut self, tx_hash: TxHash) -> TransactionEvents -> Creates a new subscription for a specific transaction hash
// 3) subscribe_all(&mut self) -> AllTransactionsEvents<T> -> Creates a new subscription for all transactions
// 4) pending(&mut self, tx: &TxHash, replaced: Option<Arc<ValidPoolTransaction<T>>>) -> Notifies listeners that a transaction was added to the pending 
//    queue
// 5) replaced(&mut self, tx: Arc<ValidPoolTransaction<T>>, replaced_by: TxHash) -> Notifies listeners that a transaction was replaced
// 6) queued(&mut self, tx: &TxHash) -> Notifies listeners that a transaction was added to the queued pool
// 7) propagated(&mut self, tx: &TxHash, peers: Vec<PropagateKind>) -> Notifies listeners that a transaction was propagated
// 8) discarded(&mut self, tx: &TxHash) -> Notifies listeners that a transaction was discarded
// 9) mined(&mut self, tx: &TxHash, block_hash: B256) -> Notifies listeners that a transaction was mined
