...
/// Errors that can occur during state processing.
#[derive(Debug, PartialEq)]
pub enum Error {
    /// Error originating from `BeaconState`.
    BeaconStateError(BeaconStateError),
    /// Error during epoch processing.
    EpochProcessingError(EpochProcessingError),
    /// Arithmetic error.
    ArithError(ArithError),
    /// Inconsistent state fork error.
    InconsistentStateFork(InconsistentFork),
}

impl From<ArithError> for Error {
    /// Converts an `ArithError` into `Error`.
...
///
/// Returns `Ok(Some(summary))` if per-epoch processing was performed, otherwise `Ok(None)`.
...
/// Caches the state and updates necessary roots.
///
/// Updates the state root and block root for the given `state`.
...
impl From<BeaconStateError> for Error {
    /// Converts a `BeaconStateError` into `Error`.
...
impl From<EpochProcessingError> for Error {
    /// Converts an `EpochProcessingError` into `Error`.
