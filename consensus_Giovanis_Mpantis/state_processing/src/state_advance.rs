...
/// Errors that may occur during state advancement.
#[derive(Debug, PartialEq)]
pub enum Error {
    /// The target slot is less than the current state slot.
    BadTargetSlot {
        /// The target slot that was attempted to be reached.
        target_slot: Slot,
        /// The current slot of the state.
        state_slot: Slot,
    },
    /// Error from per-slot processing.
    PerSlotProcessing(per_slot_processing::Error),
    /// State root not provided when required.
...
/// Checks if the `target_slot` is greater than or equal to the `state_slot`. If not, returns an
/// error indicating the bad target slot.
