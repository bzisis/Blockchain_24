...
pub struct ShareableBlockchainTree<DB, E> {
    /// The actual blockchain tree wrapped in a `RwLock` for safe concurrent access.
    ...
    E: BlockExecutorProvider,
{
    ...
    /// Inserts a block into the blockchain tree with the specified validation kind.
    fn insert_block(
        &self,
    ...
    /// Connects buffered blocks to canonical hashes and finalizes the tree.
    fn connect_buffered_blocks_to_canonical_hashes_and_finalize(
        ...
        let mut tree = self.tree.write();
        let res = tree.connect_buffered_blocks_to_canonical_hashes_and_finalize(last_finalized_block);
        tree.update_chains_metrics();
        Ok(res?)
    }
    ...
    E: BlockExecutorProvider,
{
    ...
    fn pending_block_and_receipts(&self) -> Option<(SealedBlock, Vec<Receipt>)> {
        ...
        let receipts = tree.receipts_by_block_hash(pending_block.hash())?.into_iter().cloned().collect();
