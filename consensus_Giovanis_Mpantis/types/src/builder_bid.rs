...
/// A builder bid variant for different execution payload headers.
#[superstruct(
...
impl<E: EthSpec> BuilderBid<E> {
    /// Returns a reference to the execution payload header of the builder bid.
...
impl<'a, E: EthSpec> BuilderBidRefMut<'a, E> {
    /// Returns a mutable reference to the execution payload header of the builder bid.
...
/// Signed variant of the BuilderBid, including a signature.
...
impl<E: EthSpec> ForkVersionDeserialize for BuilderBid<E> {
    /// Deserialize a BuilderBid based on the specified fork name.
...
impl<E: EthSpec> ForkVersionDeserialize for SignedBuilderBid<E> {
    /// Deserialize a SignedBuilderBid based on the specified fork name.
    fn deserialize_by_fork<'de, D: Deserializer<'de>>(
        value: serde_json::value::Value,
        fork_name: ForkName,
    ) -> Result<Self, D::Error> {
...
impl<E: EthSpec> SignedBuilderBid<E> {
    /// Verifies the signature of the SignedBuilderBid using the provided chain spec.
