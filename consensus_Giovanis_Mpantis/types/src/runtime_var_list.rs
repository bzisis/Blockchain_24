...
/// A runtime variable-length list that encodes and decodes its elements using SSZ serialization.
///
/// This struct provides methods for managing a list of elements with a maximum length constraint.
...
impl<T: Encode + Decode + Clone> RuntimeVariableList<T> {
    /// Creates a new `RuntimeVariableList` from a given vector, ensuring its length does not exceed `max_len`.
    ///
    /// # Errors
    ///
    /// Returns an error if the vector's length exceeds `max_len`.
    ...
    /// Creates a `RuntimeVariableList` from a vector, truncating it to `max_len` if necessary.
    ...
    /// Converts the `RuntimeVariableList` into a vector.
    pub fn to_vec(&self) -> Vec<T> {
        self.vec.clone()
    }

    /// Returns a slice of the elements in the `RuntimeVariableList`.
    pub fn as_slice(&self) -> &[T] {
        self.vec.as_slice()
    }

    /// Returns the length of the `RuntimeVariableList`.
    pub fn len(&self) -> usize {
        self.vec.len()
    }

    /// Checks if the `RuntimeVariableList` is empty.
    pub fn is_empty(&self) -> bool {
        self.vec.is_empty()
    }

    /// Decodes a byte slice into a `RuntimeVariableList`, validating its length against `max_len`.
    ///
    /// # Errors
    ///
    /// Returns a `ssz::DecodeError` if decoding fails or if the decoded list exceeds `max_len`.
