...
///
/// This struct uses atomic variables to track the total data size and the number of blobs
...
impl BlobStoreSize {
    /// Adds the given size to the total data size.
    #[inline]
    pub(crate) fn add_size(&self, add: usize) {
        self.data_size.fetch_add(add, Ordering::Relaxed);
    }

    /// Subtracts the given size from the total data size.
    #[inline]
...
impl PartialEq for BlobStoreSize {
    /// Implements equality comparison for `BlobStoreSize`.
...
///
/// This struct contains information about the number of successfully deleted blobs and
...
mod tests {
    ...
    /// A test structure containing a dynamic store.
    ///
    /// This is used for testing purposes to ensure that the trait methods are implemented correctly.
    ///
