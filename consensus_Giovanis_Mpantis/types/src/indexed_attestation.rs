...
///
/// [Spec v0.12.1 - IndexedAttestation](https://github.com/ethereum/consensus-specs/blob/v0.12.1/specs/phase0/beacon-chain.md#indexedattestation)
#[derive(
    Derivative,
    Debug,
    Clone,
...
impl<E: EthSpec> IndexedAttestation<E> {
    /// Check if two indexed attestations constitute a double vote.
    ///
    /// Returns `true` if both attestations have the same target epoch but different data.
    ///
    /// [Spec v0.12.1 - Double Vote](https://github.com/ethereum/consensus-specs/blob/v0.12.1/specs/phase0/beacon-chain.md#doublevote)
    pub fn is_double_vote(&self, other: &Self) -> bool {
        self.data.target.epoch == other.data.target.epoch && self.data != other.data
    }

    /// Check if one indexed attestation surrounds another.
    ///
    /// Returns `true` if `self`'s source epoch is less than `other`'s source epoch, and `self`'s
    /// target epoch is greater than `other`'s target epoch.
    ///
    /// [Spec v0.12.1 - Surround Vote](https://github.com/ethereum/consensus-specs/blob/v0.12.1/specs/phase0/beacon-chain.md#surroundvote)
