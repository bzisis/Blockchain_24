...
pub fn weigh_justification_and_finalization<E: EthSpec>(
    ...
    *state.previous_justified_checkpoint_mut() = state.current_justified_checkpoint();
    ...
    // Justify the previous epoch if conditions are met
    ...
    // Justify the current epoch if conditions are met
    ...
    // Function to check if all bits in a specified range are set
    ...
    // Finalize the checkpoint based on justification bits and epochs
