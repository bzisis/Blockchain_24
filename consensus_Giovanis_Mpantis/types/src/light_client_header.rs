...
/// A LightClientHeader represents the header information used in light client updates.
#[superstruct(
...
impl<E: EthSpec> LightClientHeader<E> {
    /// Converts a signed beacon block into a light client header based on the chain specification.
    ...
    /// Decodes a light client header from SSZ bytes for a specific fork name.
    ...
    /// Decodes a light client header from SSZ bytes for a specific fork name (alias for `from_ssz_bytes`).
    ...
    /// Calculates the maximum variable length for SSZ serialization based on the fork name.
...
impl<E: EthSpec> ForkVersionDeserialize for LightClientHeader<E> {
    /// Deserializes a light client header based on a fork name and a JSON value.
...
impl<E: EthSpec> LightClientHeaderAltair<E> {
    /// Converts a signed beacon block into a light client header for Altair fork.
...
impl<E: EthSpec> LightClientHeaderCapella<E> {
    /// Converts a signed beacon block into a light client header for Capella fork.
    pub fn block_to_light_client_header(block: &SignedBeaconBlock<E>) -> Result<Self, Error> {
        let payload = block
            .message()
            .execution_payload()?
...
impl<E: EthSpec> LightClientHeaderDeneb<E> {
    /// Converts a signed beacon block into a light client header for Deneb fork.
    pub fn block_to_light_client_header(block: &SignedBeaconBlock<E>) -> Result<Self, Error> {
        let payload = block
            .message()
            .execution_payload()?
