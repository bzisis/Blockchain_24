...
/// Errors that can occur during block signature verification.
...
pub enum Error {
    ...
    /// Error related to the consensus context, likely the proposer index or block root calculation.
    ...
    D: Fn(&'a PublicKeyBytes) -> Option<Cow<'a, PublicKey>>,
{
    ...
    /// add signatures, and the `verify` function to perform verification.
    pub fn new(
        state: &'a BeaconState<E>,
        get_pubkey: F,
        decompressor: D,
    ...
    /// Verify all the signatures that have been included in `self`, returning `Ok(())` if and only if
...
impl<'a> ParallelSignatureSets<'a> {
    /// Push a `SignatureSet` onto the list of sets to be verified.
    ...
    /// ## Notes
    ///
    /// Signature validation will take place in accordance with the [Faster verification of multiple
