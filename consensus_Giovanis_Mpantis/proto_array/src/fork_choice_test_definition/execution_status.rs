...
/// Defines a test scenario for fork choice.
pub fn get_execution_status_test_definition_01() -> ForkChoiceTestDefinition {
    ...
    // Return the finalized test definition.
    ForkChoiceTestDefinition { ops }
}
/// Builds a set of operations for testing fork choice logic.
fn build_fork_choice_test_definition() -> ForkChoiceTestDefinition {
    ...
    // Assert weights after head assertion.
    ops.push(Operation::AssertWeight {
        block_root: get_root(0),
        weight: 2,
    });
    ...
    // Assert weights after invalidation.
    ops.push(Operation::AssertWeight {
        block_root: get_root(0),
        weight: 1,
    });
    ...
    // Assert weights after reevaluation.
    ops.push(Operation::AssertWeight {
        block_root: get_root(0),
        weight: 1,
    });
...
/// Generates a fork choice test definition for execution status.
...
/// Constructs a series of operations to test fork choice logic.
pub fn get_execution_status_test_definition_02() -> ForkChoiceTestDefinition {
...
/// Constructs a series of operations to test execution status with block additions.
...
/// Constructs a test scenario for the fork choice algorithm with operations ensuring
/// correct block head selection and weight assertions.
fn get_execution_status_test_definition_01() -> ForkChoiceTestDefinition {
    let mut ops = Vec::new();
