...
/// Maximum number of subnet IDs.
const MAX_SUBNET_ID: usize = 64;

lazy_static! {
    /// Mapping from subnet ID to string representation.
...
/// Struct representing a subnet identifier.
#[derive(
...
/// Converts a subnet ID into its string representation.
...
impl SubnetId {
    /// Creates a new `SubnetId` instance with the given ID.
    pub fn new(id: u64) -> Self {
        id.into()
    }

    /// Compute the subnet ID for an attestation with `attestation_data` where each slot in the
    ...
    /// Compute the subnet ID for an attestation with `slot` and `committee_index` where each slot
    /// in the attestation epoch contains `committee_count_at_slot` committees.
...
mod tests {
    ...
    /// Unit test for `compute_subnets_for_epoch` function.
    #[test]
