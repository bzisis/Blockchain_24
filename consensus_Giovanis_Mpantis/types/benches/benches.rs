...
/// Generates a BeaconState with the given number of validators.
///
/// # Arguments
///
/// * `validator_count` - The number of validators to include in the state.
///
/// # Returns
///
/// Returns a BeaconState with the specified validators.
fn get_state<E: EthSpec>(validator_count: usize) -> BeaconState<E> {
    ...
    // Add balances for each validator
    ...
    // Create validators in parallel and add them to the state
...
/// Defines all benchmarks related to BeaconState operations.
///
/// # Arguments
///
/// * `c` - The Criterion struct used to define benchmarks.
fn all_benches(c: &mut Criterion) {
    ...
    // Benchmark encoding BeaconState to SSZ bytes
    ...
    // Benchmark decoding SSZ bytes to BeaconState
    g.bench_with_input(
    ...
    // Benchmark updating tree hash cache of initialized BeaconState
    let inner_state = state.clone();
    g.bench_with_input(
        BenchmarkId::new(
    ...
    // Benchmark updating tree hash cache of non-initialized BeaconState
    ...
    // Benchmark updating tree hash cache of initialized BeaconState with new validators
    let inner_state = state.clone();
    g.bench_with_input(
        BenchmarkId::new(
