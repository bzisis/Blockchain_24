...
/// A LightClientFinalityUpdate represents the finality data update sent to light client nodes.
#[superstruct(
...
impl<E: EthSpec> LightClientFinalityUpdate<E> {
    /// Creates a new `LightClientFinalityUpdate` from two signed beacon blocks and other parameters.
    pub fn new(
    ...
    /// Maps a function over the variant of the finality update based on its fork name.
    ...
    /// Retrieves the slot of the attested header.
    ...
    /// Constructs a `LightClientFinalityUpdate` from SSZ bytes based on the fork name.
...
impl<E: EthSpec> ForkVersionDeserialize for LightClientFinalityUpdate<E> {
    /// Deserializes a `LightClientFinalityUpdate` based on a fork name and a JSON value.
