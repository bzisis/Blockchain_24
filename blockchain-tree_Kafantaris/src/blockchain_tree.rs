    ...
    E: BlockExecutorProvider,
{
    ...
    pub fn make_canonical(
        ...
        // Clone current block indices and buffered blocks for later comparison.
        ...
        // Check if the block is already canonical.
        ...
        if let Some(header) = canonical_header {
            ...
            // Fetch total difficulty (td) for the block.
            let td =
            ...
            // Check if the block is part of the Paris fork.
            if !self
                .externals
                .provider_factory
                .chain_spec()
        ...
        // Retrieve chain ID for the block.
        ...
        // Split the chain at the block hash to make it canonical.
        ...
        // Loop to handle all forked blocks in the tree.
        while let Some(chain_id) = self.block_indices().get_block_chain_id(&fork_block.hash) {
            // Remove and split the forked chain.
            let Some(canonical) =
        ...
        // Merge all chains into one canonical chain.
        ...
        // Handle chain notification based on whether it extends the tip.
        let chain_notification = if new_canon_chain.fork_block().hash == old_tip.hash {
            // Commit the new canonical chain to the database.
            ...
            // Handle forked canonical block that is not the tip.
            let canon_fork: BlockNumHash = new_canon_chain.fork_block();
            // Sanity check to ensure all chains point to the canonical chain.
            ...
            // Revert the canonical chain from the database.
            let old_canon_chain =
            ...
            if let Some(old_canon_chain) = old_canon_chain {
                ...
                // Log error if nothing was reverted from the database.
...
mod tests {
    ...
    /// Set up the external dependencies for the blockchain tree.
    fn setup_externals(
        exec_res: Vec<ExecutionOutcome>,
    ) -> TreeExternals<Arc<TempDatabase<DatabaseEnv>>, MockExecutorProvider> {
        // Build the chain specification with the MAINNET and Shanghai activated.
    ...
    fn setup_genesis<DB: Database>(factory: &ProviderFactory<DB>, mut genesis: SealedBlock) {
        ...
        // Insert the first 10 canonical headers.
        for i in 0..10 {
            provider
                .tx_ref()
        ...
        // Insert the stage checkpoint for finishing at block 10.
        provider
            .tx_ref()
    ...
    /// Structure to test the internal state of the blockchain tree.
    #[derive(Default, Debug)]
    struct TreeTester {
        /// Number of chains.
        chain_num: Option<usize>,
        /// Mapping of block hash to chain index.
        block_to_chain: Option<HashMap<BlockHash, BlockchainId>>,
        /// Mapping of fork block hash to child block hashes.
    ...
    impl TreeTester {
        ...
        /// Set the block to chain mapping.
    ...
    fn consecutive_reorgs() {
        ...
        // Create the chain specification with a genesis block
        let chain_spec = Arc::new(
            ChainSpecBuilder::default()
                .chain(MAINNET.chain)
                .genesis(Genesis {
        ...
        // Create the provider factory and set up the consensus and executor provider
...

        {
            ...
            // Insert the initial account state
        ...
        // Define the cost of a single transaction
        let single_tx_cost = U256::from(EIP1559_INITIAL_BASE_FEE * 21_000);

        // Create a mock transaction with the given nonce
        ...
        // Create a mock block with the given parameters
        ...
        // Insert the fork block and make it canonical
        ...
        // Insert the first canonical block and make it canonical
        assert_eq!(
        ...
        // Insert the second canonical block
        assert_eq!(
            tree.insert_block(canonical_block_2, BlockValidationKind::Exhaustive).unwrap(),
            InsertPayloadOk::Inserted(BlockStatus::Valid(BlockAttachment::Canonical))
        );

        // Insert the first sidechain block and make it canonical
        assert_eq!(
        ...
        // Revert to the first canonical block
        assert_eq!(
            tree.make_canonical(canonical_block_1.hash()).unwrap(),
            CanonicalOutcome::Committed { head: canonical_block_1.header.clone() }
        );

        // Insert the second sidechain block and make it canonical
        assert_eq!(
        ...
        // Insert the third canonical block and make it canonical
        assert_eq!(
    ...
    fn sidechain_block_hashes() {
        // Set up test data with 11 blocks and retrieve specific blocks and their execution outcomes.
        ...
        // Verify that the chain containing the forked third block has the correct block hashes.
    ...
    fn cached_trie_updates() {
        ...
        // Insert the first block and check its trie updates.
        assert_eq!(
            tree.insert_block(block1.clone(), BlockValidationKind::Exhaustive).unwrap(),
            InsertPayloadOk::Inserted(BlockStatus::Valid(BlockAttachment::Canonical))
        );
        ...
        // Insert the second block and check its trie updates.
        assert_eq!(
            tree.insert_block(block2.clone(), BlockValidationKind::Exhaustive).unwrap(),
            InsertPayloadOk::Inserted(BlockStatus::Valid(BlockAttachment::Canonical))
        );
        ...
        // Insert the fifth block and check its trie updates.
        assert_eq!(
    ...
    fn last_finalized_block_initialization() {
        ...
        // Set up external dependencies with the execution outcomes, popping from last to first.
        let externals =
            setup_externals(vec![exec3.clone(), exec2.clone(), exec1.clone(), exec3, exec2, exec1]);

        // Clone the externals to use them in new tree instances later.
        ...
        // The last finalized block would be number 9. Set up the genesis block and initial state.
        setup_genesis(&externals.provider_factory, genesis);
        ...
        // Insert blocks 1, 2, and 3, ensuring they are recognized as canonical.
        assert_eq!(
            tree.insert_block(block1.clone(), BlockValidationKind::Exhaustive).unwrap(),
            InsertPayloadOk::Inserted(BlockStatus::Valid(BlockAttachment::Canonical))
        );
        ...
        // Make block 2 canonical.
        tree.make_canonical(block2.hash()).unwrap();

        // Restart the tree using the cloned externals.
        let mut tree =
            BlockchainTree::new(cloned_externals_1, config, None).expect("failed to create tree");

        // Verify that the last finalized block is 0 after restart.
        assert_eq!(tree.block_indices().last_finalized_block(), 0);

        // Create a fork of block 1 with a different hash.
        ...
        // Make the forked block canonical and finalize it.
        ...
        // Verify that the last finalized block is the number of the forked block after restart.
