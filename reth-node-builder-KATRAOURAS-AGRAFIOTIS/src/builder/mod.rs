...
/// The adapter type for a reth node with the built-in provider type
...
/// that gives access to internals of the node that are needed to configure the components. This includes
...
/// with the [NodeBuilder::launch] method. On launch, all the built-in internals, such as the
...
/// generic adapter types that then map to traits with associated types for ease of use.
///
/// ### Limitations
///
