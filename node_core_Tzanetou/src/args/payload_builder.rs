//! This module defines CLI arguments and their parsing logic for configuring parameters related to the Payload Builder.
//! 
//! The `PayloadBuilderArgs` struct specifies options for setting block extra data, target gas limit, payload build interval,
//! deadline for payload resolution, and maximum tasks for payload building. It utilizes the `clap` library for argument parsing,
//! incorporating custom parsers for validating values and ensuring compliance with constraints such as maximum extra data size.
//! 
//! The `ExtradataValueParser` is a custom parser ensuring that the block extra data size does not exceed a specified limit.
//! 
//! Tests are included to verify the correct parsing of arguments, validation of constraints, and default behavior of the CLI options.
//! These tests cover scenarios such as setting valid and invalid maximum tasks, verifying default extra data values, and handling
//! errors when extradata exceeds the maximum allowable size.
//!
