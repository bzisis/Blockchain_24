...
pub const BLOCK_NUMS_REPLAYED_TX: [u64; 6] = [
    TX_BLOCK_985[0], // First replayed block for TX_BLOCK_985
    TX_BLOCK_985[1], // Second replayed block for TX_BLOCK_985
    TX_BLOCK_123_322, // Single replayed block for TX_BLOCK_123_322
    TX_BLOCK_1_133_328[0], // First replayed block for TX_BLOCK_1_133_328
    TX_BLOCK_1_133_328[1], // Second replayed block for TX_BLOCK_1_133_328
    TX_BLOCK_1_244_152, // Single replayed block for TX_BLOCK_1_244_152
];

/// Returns `true` if the block number corresponds to a block where a transaction was replayed.
/// The blocks with replayed transactions contain only one transaction each.
pub fn is_dup_tx(block_number: u64) -> bool {
    // If the block number is greater than the last block in BLOCK_NUMS_REPLAYED_TX,
    // then there are no more blocks with replayed transactions.
    if block_number > BLOCK_NUMS_REPLAYED_TX[5] {
        return false;
    }

    // Check if the block number is in the list of blocks with replayed transactions.
    // These blocks each have exactly one transaction replayed.
