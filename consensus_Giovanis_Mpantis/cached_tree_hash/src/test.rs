...
/// Generates a vector of `Hash256` instances from `start` to `end`.
fn int_hashes(start: u64, end: u64) -> Vec<Hash256> {
    (start..end).map(Hash256::from_low_u64_le).collect()
}

/// Type alias for a `VariableList` of `Hash256` with a length of 16.
type List16 = VariableList<Hash256, U16>;
/// Type alias for a `FixedVector` of `Hash256` with a length of 16.
type Vector16 = FixedVector<Hash256, U16>;
/// Type alias for a `FixedVector` of `u64` with a length of 16.
type Vector16u64 = FixedVector<u64, U16>;

/// Tests the behavior of `TreeHashCache` when reaching maximum number of leaves.
#[test]
fn max_leaves() {
...
/// Tests that a `VariableList` of `Hash256` cannot shrink in size.
#[test]
fn cannot_shrink() {
    let arena = &mut CacheArena::default();
    let init_len = 12;
...
/// Tests the behavior of `TreeHashCache` when dealing with empty leaves.
#[test]
fn empty_leaves() {
    let arena = &mut CacheArena::default();
    let depth = 20;
...
/// Tests the `TreeHash` implementation for `FixedVector` of `Hash256`.
#[test]
...
/// Tests the `TreeHash` implementation for `FixedVector` of `u64`.
#[test]
fn fixed_vector_u64() {
    let arena = &mut CacheArena::default();
    let len = 16;
...
/// Tests the `TreeHash` implementation for `VariableList` of `Hash256`.
#[test]
...
/// Property-based test using `quickcheck` for `VariableList` of `Hash256` with a length of 256.
#[quickcheck]
fn quickcheck_variable_list_h256_256(leaves_and_skips: Vec<(u64, bool)>) -> bool {
    variable_list_h256_test::<U256>(leaves_and_skips)
}

/// Property-based test using `quickcheck` for `VariableList` of `Hash256` with a length of 255.
#[quickcheck]
fn quickcheck_variable_list_h256_255(leaves_and_skips: Vec<(u64, bool)>) -> bool {
    variable_list_h256_test::<U255>(leaves_and_skips)
}

/// Property-based test using `quickcheck` for `VariableList` of `Hash256` with a length of 257.
#[quickcheck]
fn quickcheck_variable_list_h256_257(leaves_and_skips: Vec<(u64, bool)>) -> bool {
    variable_list_h256_test::<U257>(leaves_and_skips)
}

/// Helper function for property-based tests to verify `TreeHash` behavior of `VariableList` of `Hash256`.
