...
pub struct Validator {
    ...
    /// Flag indicating if the validator has been slashed.
    pub slashed: bool,
    /// Epoch when the validator becomes eligible for activation.
    pub activation_eligibility_epoch: Epoch,
    /// Epoch when the validator was activated.
    pub activation_epoch: Epoch,
    /// Epoch when the validator initiated the exit process.
    pub exit_epoch: Epoch,
    /// Epoch when the validator's funds can be withdrawn.
...
impl Validator {
    ...
    pub fn could_be_eligible_for_activation_at(&self, epoch: Epoch, spec: &ChainSpec) -> bool {
        self.activation_epoch == spec.far_future_epoch && self.activation_eligibility_epoch < epoch
    }
    ...
    pub fn get_eth1_withdrawal_address(&self, spec: &ChainSpec) -> Option<Address> {
        self.has_eth1_withdrawal_credential(spec)
            .then(|| self.withdrawal_credentials.as_bytes().get(12..).map(Address::from_slice))
            .flatten()
    }
...
/// Checks if `withdrawal_credentials` has an 0x02 prefixed "compounding" withdrawal credential.
