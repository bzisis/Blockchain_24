    ...
    Provider: DatabaseProviderFactory<DB> + Clone + Send + Sync + 'static,
{
    ...
    pub async fn incremental_root(self) -> Result<B256, AsyncStateRootError> {
        // Call the main calculation function with retain_updates set to false
    ...
    pub async fn incremental_root_with_updates(
        ...
        // Call the main calculation function with retain_updates set to true
        self.calculate(true).await
    }

    /// Main calculation function to compute the state root.
    async fn calculate(
        self,
        retain_updates: bool,
    ) -> Result<(B256, TrieUpdates), AsyncStateRootError> {
        // Initialize the tracker to monitor the process
        let mut tracker = ParallelTrieTracker::default();
        // Construct prefix sets and freeze them for use
        let prefix_sets = self.hashed_state.construct_prefix_sets().freeze();
        // Determine storage root targets from the hashed state
        let storage_root_targets = StorageRootTargets::new(
            self.hashed_state.accounts.keys().copied(),
            prefix_sets.storage_prefix_sets,
        );
        // Sort the hashed state for consistent access
        let hashed_state_sorted = Arc::new(self.hashed_state.into_sorted());

        // Pre-calculate storage roots asynchronously for accounts with changes
        ...
        while let Some(node) = account_node_iter.try_next().map_err(ProviderError::Database)? {
            match node {
                ...
                TrieElement::Leaf(hashed_address, account) => {
                    // Handle leaf nodes by computing or retrieving storage roots
