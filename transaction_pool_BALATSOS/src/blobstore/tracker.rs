...
///
/// This struct maintains a record of blob transactions included in blocks,
/// allowing for tracking and finalizing transactions as blocks are processed.
///
...
impl BlobStoreCanonTracker {
    ...
    ///
    /// This method records the blob transactions included in a specific block.
    ///
    /// # Arguments
    ///
    /// * `block_number` - The number of the block being added.
    /// * `blob_txs` - An iterable collection of blob transaction hashes.
    ///
    pub fn add_block(
        &mut self,
...
///
/// This enum represents the possible updates that can be applied to the blob store
/// as a result of block finalization.
///
...
mod tests {
    use super::*;
    /// Tests the functionality of the `BlobStoreCanonTracker`.
    #[test]
